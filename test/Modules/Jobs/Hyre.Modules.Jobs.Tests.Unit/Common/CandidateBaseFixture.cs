...
public abstract class CandidateBaseFixture : BaseFixture
{
	...
	protected ICollection<CandidateEducation> GenerateCandidateEducation(int count)
	{
	...
	protected IEnumerable<CandidateExperience> GenerateCandidateExperience(int count)
	{
		var startDate = DateOnly.FromDateTime(DateTime.Now.AddYears(-5));
		var endDate = DateOnly.FromDateTime(DateTime.Now.AddYears(-10));
		return Enumerable
