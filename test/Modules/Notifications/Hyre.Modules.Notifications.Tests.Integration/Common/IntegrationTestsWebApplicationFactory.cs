...
public sealed class IntegrationTestsWebApplicationFactory : WebApplicationFactory<IBootstrapperMarker>, IAsyncLifetime
{
	...
	protected override void ConfigureWebHost(IWebHostBuilder builder) => builder
		...
		{
			var descriptor = services.SingleOrDefault(d => d.ServiceType == typeof(DbContextOptions<NotificationsRepositoryContext>));
			if (descriptor is not null)
			{
				_ = services.Remove(descriptor);
			}

			_ = services.AddDbContext<NotificationsRepositoryContext>(options => options.UseNpgsql(_dbContainer.GetConnectionString())
